...
import pytest
from waterflow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, convert_kPa_to_psi
...
def test_pressure_gain_from_water_height():
    ...
    assert pressure_gain_from_water_height(0.0) == approx((0.000, 0.000), abs=1)
    assert pressure_gain_from_water_height(30.2) == approx((295.628, 42.900), abs=1)
    assert pressure_gain_from_water_height(50.0) == approx((489.450, 71.000), abs=1)
...
def test_convert_kPa_to_psi():
    ...
    assert convert_kPa_to_psi(0.0) == approx(0.0, abs=0.1)
    assert convert_kPa_to_psi(100.0) == approx(14.5038, abs=0.1)
    assert convert_kPa_to_psi(250.0) == approx(36.2594, abs=0.1)
    assert convert_kPa_to_psi(489.45) == approx(71.000, abs=0.1)
